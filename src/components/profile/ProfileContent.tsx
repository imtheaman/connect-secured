...
const ProfileContent: React.FC = () => {
  ...
  const {
    data: {
      getProfile: { name, _id, about, email, profilePic, lastActive }
    }
  } = useQuery(GET_PROFILE, { variables: { userId } });
  return (
    <div
    ...
    >
      <div className='absolute -top-14 left-1/2 -translate-x-1/2'>
        <ProfilePic profilePic={profilePic} width='120px' height='120px' />
      </div>
      <div className='text-center'>
        <h1 className='font-semibold text-lg'>{name}</h1>
        <p
          className={`text-xs inline-block px-2 rounded-md py-[0.1rem] bg-gray-100 font-medium ${
            lastActive?.online ? 'text-green-500' : 'text-gray-400'
          }`}
        >
          {lastActive?.online
            ? 'Online'
            : `last active ${calculateTimeAgo(lastActive)}`}
        </p>
      </div>
      <div className='p-6 pt-0 mt-6 space-y-4 overflow-y-scroll' id='about'>
        ...
        <div className='flex items-center'>
          ...
          <div className='ml-2'>
            <p className='text-xs text-gray-500'>Connect Id</p>
            <p className='-mt-1'>@{_id}</p>
          </div>
        </div>
        {/* Settings */}
