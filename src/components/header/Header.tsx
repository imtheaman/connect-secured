...
const Header: React.FC<{ setSearch: Dispatch<SetStateAction<boolean>> }> = ({
  ...
  const user: {name: string, profilePic: string} = JSON.parse(localStorage.getItem('user') || '')
      ...
      <div className='flex justify-between p-6'>
        <div>
          ...
          <h1 className='font-semibold text-light-black capitalize -mt-1 text-xl'>
            {user.name.slice(0, 7)}{user.name.length > 8 && '...'}
          </h1>
        </div>
        ...
        <div className='flex space-x-4 items-center'>
          ...
          >
            {user.profilePic.startsWith('blob') ? <div className='flex items-center justify-center border-2 bg-white rounded-full w-9 h-9'><svg xmlns="http://www.w3.org/2000/svg" className='w-4 h-4 mb-[.2rem]' fill='#5a5b5c' viewBox="0 0 512 512"><path d="M256 288c79.53 0 144-64.47 144-144s-64.47-144-144-144c-79.52 0-144 64.47-144 144S176.5 288 256 288zM351.1 320H160c-88.36 0-160 71.63-160 160c0 17.67 14.33 32 31.1 32H480c17.67 0 31.1-14.33 31.1-32C512 391.6 440.4 320 351.1 320z"/></svg></div> :<img src={user.profilePic} className='rounded-full object-contain' />}
          </button>
        </div>
      </div>
